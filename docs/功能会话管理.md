...
- 会话管理面板顶部 Tab 切换：Claude Code / Open Code / Pi Agent（后两者暂未实现，置灰不可选）
- 按**项目**维度折叠分组展示会话列表
- 每行展示：会话标题（`display`）、时间（`timestamp`）、`sessionId`、**恢复** 按钮
- 支持按项目路径、标题、时间文本搜索
- 恢复会话：在项目目录下新建终端 Tab，执行 `claude -r {sessionId}`
- Open Code、Pi Agent 会话管理开关在设置中预留，当前为禁用状态
...
| 组件 / 模块 | 职责 |
|-------------|------|
| `SessionSettings.tsx` | 设置 UI：总开关、Claude Code 开关与路径、预留工具开关 |
| `SessionManagementPanel.tsx` | 面板：工具 Tab、搜索栏、按项目折叠列表、恢复按钮 |
...
- Open Code、Pi Agent 的解析与恢复逻辑尚未实现，设置中对应开关为禁用状态。
