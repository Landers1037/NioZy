...
本地 Git 仓库的添加、拉取、分支切换，以及提交图与 diff 浏览。
...
| `RepoListView.tsx` | 仓库列表、添加 / 拉取 / 移除 |
...
export class GitService {
  // resolveGitPath / detectGit / validateRepo / pickDirectory
...
`electron/shared/repo-types.ts` — `ManagedRepo`、`ManagedRepoSummary`、`GitGraphRow`、`GitCommitDetail` 等。
