...
存储于 `shortcuts.app`，在 `useAppShortcuts` 中监听，包括：复制/粘贴、行首/行尾、清空终端、新建终端、打开设置、上/下一个终端 Tab 等。默认值见 `DEFAULT_SHORTCUTS`。
...
`src/hooks/useAppShortcuts.ts` — 在 `App.tsx` 挂载，监听 `shortcuts.app` 各绑定。
