...
- 明暗主题 `light` / `dark`
- UI 风格：`minimal` | `niozy` | `windowsClassic` | `waFu` | `cyberpunk` | `glass`（玻璃，半透明毛玻璃）| `claude`（Claude 暖奶油白编辑风）
...
`src/components/settings/AppearanceSettings.tsx`（动画开关、布局模式、UI 风格等）

国际化：`src/lib/i18n.ts`、`src/locales/zh.json` 等（`uiStyle.glass`、`settings.appearance.enableDialogAnimations`、`tab.addToGroup`、`tab.enterGroup`、`tab.closeGroup` 等键）。
