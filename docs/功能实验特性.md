...
sequenceDiagram
  ...
  Host->>PTY: setActiveStreams + resize
  PTY-->>X: terminal:data 续流
```

**说明**：Tab 切换**立即** commit，不再等待 dwell；`attachPtyTabSwitchDwellMs` 仅用于 detach 时是否保存快照（停留不足则跳过，依赖主进程 `pausedOutput` 续流）。
...
- `src/lib/attach-pty-scrollback-offload.ts` — scrollback 侧存储
...
Attach-PTY 主要优化**多 Tab 内存**；单 Tab 疯狂打日志时 CPU 仍取决于整条输出管线。当前实现：

```mermaid
flowchart LR
  PTY["PTY / SSH"] --> Flush["主进程 flusher\n16ms 合并 + 256KB 上限"]
...
| 主进程暂停缓冲 | `electron/terminal-service.ts` | 非活跃流 `pausedOutput`，上限 512KB |
...
| Shell 高亮 | `src/lib/terminal-shell-addons.ts` | 链接/日志高亮 100ms 节流；viewport 未变时跳过重绘 |

**注意**：pending 超限时旧输出会被丢弃（保留最新内容），以避免 OOM 与 xterm `write data discarded`；极端日志洪水场景仍建议在远端限流或调低 `terminal.scrollback`、关闭 Shell「日志级别着色」。
