...
- **现代风格 UI**（可选）：左侧文件树 + 右侧文件浏览器，参考资源管理器 / NAS Web 文件管理器
  - 左侧顶部 **收藏栏**：展示已收藏的目录，点击跳转；支持右键菜单（与目录树相同，含「从收藏中移除」）；区域最大高度为左侧面板的 50%，超出后内部滚动
  - 右侧初始展示「此电脑」与本地磁盘驱动器（C:、D: 等）
  - 右侧目录右键菜单支持 **添加到收藏** / **从收藏中移除**
...
| 层级 | 文件 |
|------|------|
| **主进程** | `electron/fs-service.ts`、`electron/filesystem-favorites-store.ts`、`electron/open-directory.ts`、`electron/local-file-protocol.ts`、`electron/ssh-service.ts`（`listLocalDirectory`） |
...
flowchart TB
  ...
  Favorites["FilesystemFavoritesSection"]
  Browser["FilesystemBrowserPane"]
  API["fs:listRoots · fs:listFavorites · ssh:listLocal · getImagePreviewUrl · openWithProgram"]
  Store["filesystem-favorites-store"]
...
| 现代 | `ModernFilesystemPanel` | 左右分栏；左侧收藏栏 + 目录树，右侧完整文件列表 |
...
| 文件 | 说明 |
|------|------|
| `%USERPROFILE%/.config/NioZy/filesystem-favorites.json` | 现代 UI 收藏目录列表（`FilesystemFavoritesStore`） |
| `settings.json` → `filesystem` | 程序路径、现代 UI 开关等（见上） |
  ...
  "favorites": [
    {
      ...
      "path": "D:\\code\\project",
      "displayName": "project",
      "addedAt": 1718600000000
    }
  ]
}
```

类型：`electron/shared/filesystem-favorites-types.ts`。
...
| `ModernFilesystemPanel.tsx` | 现代分栏主面板、收藏状态、树与浏览器同步 |
| `FilesystemTreeSidebar.tsx` | 左侧收藏栏 + 懒加载目录树 |
| `FilesystemFavoritesSection.tsx` | 收藏栏 UI（最大高度 50% 面板） |
| `FilesystemBrowserPane.tsx` | 右侧浏览器（面包屑、上一级、刷新、列表/网格） |
| `FilesystemEntryContextMenu.tsx` | 共享右键菜单（终端、收藏、VS Code、Cursor、自定义） |
...
ipcMain.handle('fs:addFavorite', (_, path) => filesystemFavoritesStore.add(path))
ipcMain.handle('fs:removeFavorite', (_, id) => ({ ok: filesystemFavoritesStore.remove(id) }))
