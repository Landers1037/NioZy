...
- 初始化页：居中圆角对话框，胶囊式工作目录与开发工具选择（工具名左侧品牌图标）+ 自定义命令输入 + Start
...
| 层级 | 文件 |
|------|------|
| **主进程** | `electron/workspace-service.ts`（目录列表、Git 状态/diff）、`electron/git-service.ts`（解析 git 路径）、`electron/terminal-service.ts`（`claimStream`） |
...
| `WorkspaceInitView.tsx` | 胶囊目录/工具下拉（带品牌图标）、命令输入框、Start |
...
- `workspace:getHomeDir` / `listDir` / `pickDirectory` / `detectGit` / `gitStatus` / `gitDiff`
...
Store 方法：`startWorkspace`（标记启动）、`ensureWorkspaceTerminal`（创建 PTY，由 `WorkspaceActiveView` 挂载时调用）。
