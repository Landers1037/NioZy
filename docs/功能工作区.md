...
- 侧栏激活工作区 Tab 支持右键修改标题；未自定义时默认显示「工作区」或 Start 后的目录名
...
- 文件树：懒加载子目录，按 Tab 缓存已加载目录，切换 Tab 后可快速恢复；点击 `.md` / `.markdown` 文件可复用 Markdown 编辑器能力打开预览
- Git 工作区（可选开关）：未提交文件按新增 / 修改 / 删除分组，显示 `+N -M` 行数；点击文件查看 unified diff；新增/删除行数分别用绿色/红色区分
- Git 工作区顶部：刷新、提交、推送按钮，以及显示当前分支的 Tag
- Git 提交：弹框中输入 commit message，并通过复选框勾选本次要提交的文件；仅提交已勾选文件
- Git 路径与文件名：Git 命令使用 `core.quotepath=false`，中文等非 ASCII 路径正常显示；未跟踪新增文件从磁盘读取内容生成 diff，删除文件也支持查看 diff
...
| 层级 | 文件 |
|------|------|
| **主进程** | `electron/workspace-service.ts`（目录列表、Git 状态/diff/branch/commit/push）、`electron/workspace-history-store.ts`（历史工作区持久化）、`electron/git-service.ts`（解析 git 路径）、`electron/terminal-service.ts`（`claimStream`） |
...
flowchart TB
  ...
  GitView -->|gitStatus / gitDiff / gitBranch / gitCommit / gitPush| WsSvc
...
sequenceDiagram
  ...
  participant API as workspace:gitStatus / gitDiff / gitBranch / gitCommit / gitPush
  participant Ws as WorkspaceService
  participant Git as git CLI
  participant Diff as WorkspaceDiffDialog

  UI->>API: gitStatus(workDir) + gitBranch(workDir)
  Ws->>Git: -c core.quotepath=false status --porcelain -uall
  Ws->>Git: diff --numstat HEAD + diff --cached --numstat
  Ws->>Git: branch --show-current / rev-parse --abbrev-ref HEAD
  API-->>UI: 文件列表 + 增删行数 + 当前分支（UTF-8 路径）
  ...
  Note over UI: 用户点击 Commit
  UI->>UI: 输入 message + 勾选文件
  UI->>API: gitCommit(workDir, message, filePaths[])
  Ws->>Git: 临时 index 仅 add 勾选文件 → commit-tree / update-ref HEAD
  API-->>UI: 成功后刷新 branch / status

  Note over UI: 用户点击 Push
  UI->>API: gitPush(workDir)
  Ws->>Git: push
```
...
interface AppTab {
  ...
  customTitle?: string // 右键编辑后的自定义标题；为空则回退到 title
  workspaceDir?: string
  terminalId?: string
}
```
...
| `WorkspaceFileTree.tsx` | 递归目录树，懒加载，缓存于 `sessions[tabId].fileTreeCache`；Markdown 文件点击后打开预览 |
| `WorkspaceGitView.tsx` | 未提交文件分类列表、分支 Tag、刷新 / 提交 / 推送按钮、选择性提交弹框 |
| `WorkspaceDiffDialog.tsx` | 文件 diff 对话框，复用 `CommitFileDiffView` |
| `WorkspaceTabItem.tsx` | 侧栏 Tab 项（Computer / Sparkle）；支持右键修改标题；已启动时关闭需确认 |
...
| `getGitBranch(workDir)` | 获取当前分支（`branch --show-current` 等回退链） |
| `gitDiff(workDir, filePath)` | 工作区/暂存区 diff；暂存新增用 `show :0:path`；**未跟踪**新增从磁盘 `readFile` 生成 unified diff |
| `gitCommit(workDir, message, filePaths)` | 基于临时 index 仅提交勾选文件，最后同步选中文件到真实 index |
| `gitPush(workDir)` | 执行 `git push` |
...
- `workspace:gitBranch` / `gitCommit` / `gitPush`
...
| 侧栏 Tab 列表 | `WorkspaceTabItem.tsx` | 展示各工作区 Tab；Start 后标题为目录名；支持右键编辑自定义标题；图标切为 `Sparkle` |
...
Store 方法：`startWorkspace`（标记启动并写入历史）、`ensureWorkspaceTerminal`（创建 PTY，由 `WorkspaceActiveView` 挂载时调用）、`restoreFromHistory`（从历史恢复并 Start）、`recordWorkspaceHistory`、`refreshGitStatus`、`refreshGitBranch`。

## 类型定义

`electron/shared/workspace-types.ts` — `WorkspaceDirEntry`、`WorkspaceGitFile`、`WorkspaceGitFileStatus`、`WorkspaceListDirResponse`、`WorkspaceGitStatusResponse`、`WorkspaceGitDiffResponse`、`WorkspaceDetectGitResponse`、`WorkspaceGitBranchResponse`、`WorkspaceGitCommitResponse`、`WorkspaceGitPushResponse`、`WORKSPACE_TOOL_IDS`、`WORKSPACE_TOOL_COMMANDS`。
