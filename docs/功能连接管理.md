# 功能：连接管理

自定义连接（本地命令、SSH、RDP、PuTTY、VNC 等）的 CRUD 与一键连接。

## 功能列表

- 自定义连接列表增删改查、排序
- 内置 Shell 开关（PowerShell / CMD / pwsh）
- 默认启动 Shell 类型
- 从侧栏/菜单新建：终端、SSH、RDP、PuTTY、VNC（实验）
- 从侧栏/菜单新建：终端、SSH、SFTP、FTP、RDP、PuTTY、VNC（实验）
- RDP：调用 `mstsc`
- PuTTY：调用 `putty.exe`
- VNC Web Viewer Tab（实验，见 [功能实验特性.md](./功能实验特性.md)）
- 连接草稿与 Vault 变量引用 `${KEY}`
- **SSH 动态密码**：创建/编辑 SSH 连接时，在「认证方式」右侧可开启「动态密码」（仅密码认证有效）；连接前弹框输入服务器当前随机密码，实际登录密码为配置密码与其拼接（见 [功能SSH连接.md](./功能SSH连接.md)）
- **SSH 连接后脚本**：创建/编辑 SSH 连接时可配置多行 bash 命令，连接成功后按行依次执行（内置 ssh2 与系统 `ssh.exe` 均支持；断线重连后也会执行）
- **自定义命令右键打开**（仅 Windows）：在创建/编辑自定义命令时，名称行可开启「右键打开」，向资源管理器注册「通过 NioZy 打开 {名称}」，在该目录下启动对应自定义命令

## 进程归属

| 连接类型 | 主进程 | 渲染层 |
|----------|--------|--------|
| 本地/SSH 终端 | `terminal-service` | `terminal-actions` |
| FTP 面板 | `electron/ftp-service.ts` | `src/components/scp/FtpPanel.tsx` |
| RDP | `electron/rdp-launch.ts` | `ConnectionSettings` |
| PuTTY | `electron/putty-launch.ts` | 同上 |
| VNC | `electron/vnc-proxy.ts` | `src/components/vnc/VncPanel.tsx` |
| 自定义命令右键菜单 | `electron/windows-connection-context-menu.ts` | `ConnectionSettings` |

## 架构与数据流

### 连接类型路由

```mermaid
flowchart TB
  List["ConnectionSettings / 新建菜单"]
  termjson[("term.json")]
  List --> termjson

  List -->|type=builtin| Local["terminal-actions → PTY"]
  List -->|type=ssh| SSH["terminal:create → ssh2 / ssh.exe"]
  List -->|type=ftp| FTP["ftp-service → FtpPanel"]
  List -->|type=ssh + sshDynamicPassword| DynPwd["SshDynamicPasswordDialog → sshDynamicPasswordSuffix"]
  List -->|type=rdp| RDP["rdp-launch → mstsc"]
  List -->|type=putty| PuTTY["putty-launch"]
  List -->|type=vnc + 实验开关| VNC["vnc-proxy + VncPanel Tab"]
  List -->|type=command + shellContextMenu| Reg["windows-connection-context-menu → 注册表"]
  List -->|type=ssh + sshStartupScript| Script["ssh-startup-script → terminal write"]
  DynPwd --> SSH
```

### SSH 动态密码

```mermaid
sequenceDiagram
  participant UI as ConnectionSettings
  participant Draft as connection-draft
  participant Disk as term.json
  participant Menu as 侧栏 / 新建菜单
  participant TA as terminal-actions
  participant Prompt as SshDynamicPasswordDialog
  participant API as terminal:create

  UI->>Draft: sshDynamicPassword 开关（认证方式右侧）
  Draft->>Disk: 保存 sshDynamicPassword: true（不含每次输入的值）
  Menu->>TA: createConnection(ssh 连接)
  TA->>Prompt: 弹框输入动态密码
  Prompt-->>TA: 后缀（取消则中止）
  TA->>API: terminal.create({ sshConnectionId, sshDynamicPasswordSuffix })
  API->>API: resolveSshConnectionPassword(sshPassword + 后缀)
```

- 配置字段：`CustomConnection.sshDynamicPassword`（布尔，持久化于 `term.json`）
- UI：`ConnectionSettings.tsx` 中 SSH 表单的「认证方式」Select 右侧 Switch；切换为公钥认证时自动关闭
- 临时传参：`TerminalCreateOptions.sshDynamicPasswordSuffix`（IPC 一次性传入，不写入 Tab `terminalSpawn`）
- 草稿映射：`src/lib/connection-draft.ts` — `connectionToDraft` / `draftToConnection`

### SSH 连接后脚本

```mermaid
sequenceDiagram
  participant UI as ConnectionSettings
  participant API as terminal:create
  participant TS as TerminalService
  participant Script as ssh-startup-script
  participant Remote as SSH Shell

  UI->>API: 打开 SSH 连接（含 sshStartupScript）
  API->>TS: createSsh2 / PTY ssh
  TS->>Remote: 交互式 Shell 就绪
  API->>Script: scheduleSshStartupScript
  Script->>Remote: 按行写入命令（Vault 已解析）
```

- 配置字段：`CustomConnection.sshStartupScript`（多行文本，每行一条 bash 命令）
- 空白行忽略；支持 `${vaultKey}` 引用
- 执行时机：Shell 就绪后延迟约 600ms，行间间隔约 120ms
- 多行块结构（如 `if … fi`）需写成单行或使用 `bash -c '…'`

### Telnet

- 连接类型 `telnet` 不再依赖 Windows `telnet.exe`
- 启动方式改为应用内终端拉起内置 Telnet bridge，bridge 基于 npm `telnet-client`
- 无需启用 Windows「Telnet 客户端」可选功能

### FTP / FTPS

- 新增连接类型：`ftp`
- 配置字段：`CustomConnection.ftpHost`、`ftpPort`、`ftpUser`、`ftpPassword`、`ftpSecurity`、`ftpTransferMode`、`ftpTimeoutSeconds`
- 支持：
  - FTP 明文
  - FTPS 显式 TLS
  - FTPS 隐式 TLS
  - 主动模式
  - 被动模式
- 打开 FTP 连接时，在右侧区域打开独立 FTP 文件传输面板，交互与 SFTP 面板一致
- 默认超时时间：10 秒

### 保存连接配置

```mermaid
sequenceDiagram
  participant UI as ConnectionSettings
  participant Store as patchSettings
  participant Main as SettingsStore + ConnectionsStore
  participant Reg as windows-connection-context-menu
  participant Disk as term.json

  UI->>Store: patchSettings({ connections })
  Store->>Main: settings:save
  Main->>Disk: ConnectionsStore.save
  Main->>Reg: syncConnectionContextMenus（增删改注册表）
  Main-->>UI: 合并后的 AppSettings.connections
```

### 自定义命令右键打开

```mermaid
sequenceDiagram
  participant Explorer as 资源管理器右键
  participant Main as main/index.ts
  participant App as App.tsx
  participant Term as terminal-actions

  Explorer->>Main: NioZy.exe --niozy-connection={id} "%V"
  Main->>App: app:openDirectory { directory, connectionId }
  App->>Term: openConnectionInDirectory(id, cwd)
  Term->>Term: 在目录下启动自定义命令 Tab
```

- 注册表路径（每连接独立键名 `NioZy.Conn.{id}`）：
  - `HKCU\Software\Classes\Directory\shell\NioZy.Conn.{id}`
  - `HKCU\Software\Classes\Directory\Background\shell\NioZy.Conn.{id}`
- 菜单显示文案：`通过 NioZy 打开 {连接名称}`
- 关闭「右键打开」或删除连接时，主进程从注册表移除对应项
- 同名重复：若已有其他自定义命令（同名且已开启右键打开）存在，保存时提示用户，不写入注册表

## 实验特性

- **VNC Web Viewer**：`experimental.vncWebEnabled` 及相关 `vnc*` 选项

## 配置文件片段

`term.json`：

```json
{
  "connections": [
    {
      "id": "uuid",
      "name": "My Server",
      "type": "ssh",
      "command": "192.168.1.1",
      "args": [],
      "env": {},
      "sshHost": "192.168.1.1",
      "sshPort": 22,
      "sshUser": "root",
      "sshAuth": "password",
      "sshPassword": "fixed-prefix",
      "sshDynamicPassword": true,
      "sshStartupScript": "cd /var/log\nls -la"
    },
    {
      "id": "cmd-uuid",
      "name": "Dev Shell",
      "type": "command",
      "command": "pwsh.exe",
      "args": ["-NoLogo"],
      "env": {},
      "shellContextMenu": true
    },
    {
      "id": "ftp-uuid",
      "name": "Files",
      "type": "ftp",
      "command": "192.168.1.20",
      "args": [],
      "env": {},
      "ftpHost": "192.168.1.20",
      "ftpPort": 21,
      "ftpUser": "deploy",
      "ftpPassword": "${FTP_PASSWORD}",
      "ftpSecurity": "explicit",
      "ftpTransferMode": "passive",
      "ftpTimeoutSeconds": 10
    }
  ]
}
```

`settings.json`：

```json
{
  "defaultTerminal": "powershell",
  "builtinConnections": { "powershell": true, "cmd": true, "pwsh": true }
}
```

## 数据存储

| 路径 | 内容 |
|------|------|
| `%USERPROFILE%\.config\NioZy\term.json` | 全部 `CustomConnection[]` |

存储类：`electron/connections-store.ts`（`9:38:electron/connections-store.ts`）。

## 核心代码

### ConnectionsStore

```13:37:electron/connections-store.ts
  load(): CustomConnection[]
  save(connections: CustomConnection[]): CustomConnection[]
```

### 设置 UI

`src/components/settings/ConnectionSettings.tsx` — 连接表格与编辑表单（约 85 行起 `export function ConnectionSettings`）。SSH 类型在「认证方式」行提供「动态密码」开关，并提供「连接后脚本」多行输入；FTP 类型提供安全模式、主动/被动模式与超时配置；自定义命令类型在名称行提供「右键打开」开关（与名称同一行）。

### SSH 动态密码

`src/lib/connection-draft.ts` — 草稿字段 `sshDynamicPassword`；保存为 `CustomConnection.sshDynamicPassword`。

`src/lib/terminal-actions.ts` — `applySshDynamicPasswordToCreateOptions`；`src/components/ssh/SshDynamicPasswordDialog.tsx` — 连接前输入弹框。

`electron/ssh-auth.ts` — `resolveSshConnectionPassword`；`electron/main/index.ts` — `terminal:create` 传入 `sshDynamicPasswordSuffix` 后解析 profile / spawn 环境。

### SSH 连接后脚本

`electron/ssh-startup-script.ts` — `parseSshStartupScriptLines` / `scheduleSshStartupScript`。

`electron/main/index.ts` — `runSshConnectionStartupScript`，在 `terminal:create` 成功创建 SSH 会话后调用。

### FTP 面板

`electron/ftp-service.ts` — FTP 连接、目录列表、上传、下载。

`src/components/scp/FtpPanel.tsx` / `src/components/scp/SftpTransferBody.tsx` — 右侧 FTP 文件面板与共用传输 UI。

### 新建连接菜单

`src/components/layout/NewConnectionMenuContent.tsx`

### VNC Tab

`useAppStore.addVncTab`：`src/stores/app-store.ts`；面板 `src/components/vnc/VncPanel.tsx`。

### 自定义命令右键注册表

`electron/windows-connection-context-menu.ts` — `registerConnectionContextMenu` / `unregisterConnectionContextMenu` / `syncConnectionContextMenus`。

### 主进程 IPC

```1032:1076:electron/main/index.ts
ipcMain.handle('rdp:connect', async (_, connectionId: string) => { /* ... */ })
ipcMain.handle('putty:connect', async (_, connectionId: string) => { /* ... */ })
ipcMain.handle('vnc:startProxy', /* ... */)
ipcMain.handle('vnc:stopProxy', /* ... */)
```

保存连接时 `settings:save` 在 `partial.connections` 变更后调用 `syncConnectionContextMenus`；应用启动时调用 `syncAllConnectionContextMenus` 恢复注册表。
