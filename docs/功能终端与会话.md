...
- Tab 内拆分终端，最多 4 个 pane；四分屏默认布局为 2×2
- 拆分 pane 支持分隔线高亮、拖拽调整宽高，及顶部拖拽把手换位
...
| 拆分终端 | `splitTerminalTab`（未达 4 pane 上限时可用） |
...
|------|----------|
| （空） | 3 条常用命令（按历史使用频率） |
| 任意文本 | 模糊匹配命令名称、关键词、命令 id 与所属分组 |
| `/all`、`help`、`/help` | 展示全部命令；按分组插入浅色小标题，可用项在前，不可用灰显 |

**列表展示**：

- 每条命令带所属分组；常规搜索结果在右侧以浅色小字显示分组，`/all` 全量列表会按分组插入浅色分段标题
- **切换工作目录** 命令会在命令名前显示当前本地终端工作目录 chip（浅色文本 + 圆角背景，超长自动截断）
- 设置类命令会在右侧展示当前状态（如 开启 / 关闭、当前主题、当前配色）
...
| 分组 | 命令 | 说明 | 可用条件 |
|------|------|------|----------|
| 终端与会话 | 新建终端 | 调用 `createTerminal()` | 始终 |
| 终端与会话 | 切换工作目录 | 打开目录选择器；选择后以该目录新开一个**本地**终端 Tab | 始终 |
| 终端与会话 | 修改终端 Tab 标题 | 打开编辑标题对话框 | 存在终端 Tab |
| 终端与会话 | 关闭终端 Tab | 二次确认后关闭 | 存在终端 Tab |
| 终端与会话 | 添加终端 Tab 到一个分组 | 打开 `AddToGroupDialog` | 存在终端 Tab |
| 终端与会话 | 将终端 Tab 移出分组 | `removeTabFromAllGroups` | 目标 Tab 已在某分组 |
| 终端与会话 | 克隆终端 Tab | `cloneTerminalTab` | 存在终端 Tab |
| 终端与会话 | 拆分终端 | `splitTerminalTab` | 存在终端 Tab 且未达拆分上限（4 pane） |
| 终端与会话 | 导出终端 | 导出活跃 pane 缓冲区为 `.txt` | 存在终端 Tab |
| 终端与会话 | 终端截图 | 打开 `TerminalScreenshotDialog` | 存在终端 Tab |
| 终端外观 | 切换渲染模式 | 进入子面板选择 DOM / WebGL | xterm 引擎（Wterm 下不可用） |
| 终端外观 | 切换终端配色方案 | 进入子面板选择内置配色 | 始终（写入全局 `terminal.colorScheme`） |
| 终端外观 | 切换光标闪烁 | 直接切换 `terminal.cursorBlink` | 始终 |
| 终端外观 | 切换隐藏宿主光标 | 直接切换 `terminal.hideCursor` | 始终 |
| 终端外观 | 切换终端连字 | 直接切换 `terminal.ligaturesEnabled` | 始终 |
| 终端行为 | 切换右键复制/粘贴 | 直接切换 `terminal.rightClickCopyPaste`；开启时自动关闭高级右键菜单 | 始终 |
| 终端行为 | 切换高级右键菜单 | 直接切换 `terminal.advancedRightClickMenu`；开启时自动关闭右键复制/粘贴 | 始终 |
| 终端行为 | 切换同步输出 | 直接切换 `terminal.synchronizedOutputEnabled` | 始终 |
| 终端行为 | 切换终端闲置动画 | 直接切换 `terminal.idleAnimation.enabled` | 始终 |
| 终端行为 | 切换欢迎页面 | 直接切换 `terminal.welcomePage.enabled` | 始终 |
| 界面外观 | 切换界面风格 | 进入子面板选择内置界面风格 | 始终（写入全局 `uiStyle` 与对应默认 `accentColor`） |
| 界面外观 | 切换明亮/暗黑主题 | 进入子面板选择明亮 / 暗黑 | 始终（写入全局 `theme`） |
| 辅助工具 | 打开设置 | `addSettingsTab()` | 始终 |
| 辅助工具 | 切换终端搜索入口 | 直接切换 `assistive.terminalSearchEnabled` | 始终 |
| 辅助工具 | 切换命令重放入口 | 直接切换 `assistive.commandReplayEnabled` | 始终 |
| 辅助工具 | 切换番茄钟入口 | 直接切换 `assistive.pomodoroEnabled` | 始终 |
| AI | 打开/关闭 AI 对话 | `useAiSidebarStore.toggle()` | `ai.aiSidebarEnabled` 已开启 |
| AI | 新建 AI 对话 | 打开边栏（若已关闭）并 `requestNewChat()` | `ai.aiSidebarEnabled` 已开启 |
...
| `animation` | 动画类型：`niozy3d`（3D WebGPU）、`particles`（Logo 粒子动画）或 `ascii`（ASCII Text 动画，默认 `niozy3d`） |
...
| `ascii` | React Bits 风格 ASCII Text 页面：中心文本固定为 `>_ NioZy`，底层用 Three.js 平面 + shader 做轻微波动/色偏，前景用 ASCII 字符栅格实时重建文本 | 无需 WebGPU；若组件初始化失败则保持纯背景与底部提示 |
...
- **ASCII 模式额外**：深色背景上叠加渐变 ASCII 字符画；鼠标移动仅改变文本平面朝向与色相，不阻挡底层点击
...
- 视图：`WelcomePageAsciiView`
- 组件：`src/components/effects/ASCIIText.tsx`
- 样式：`src/components/effects/ascii-text.css`
- 渲染：Three.js `WebGLRenderer` + shader 平面，离屏 canvas 文本纹理，再映射到 ASCII 字符网格 `<pre>`
...
| `ascii` | ASCII Text 屏保页面：覆盖层中央显示 `>_ NioZy`，以字符网格实时生成文本并随鼠标轻微摆动/变色 |
...
- **ASCII** 模式在磨砂层上再叠一层深色氛围底，用字符画作为主视觉；仍保持 `pointer-events-none`
...
- ASCII 闲置：`src/components/effects/ASCIIText.tsx`
