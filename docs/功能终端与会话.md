...
- **无 Tab 欢迎页（WebGPU）**：无任何 Tab 时，在右侧主区域展示 3D NioZy Logo 终端动画（需开启硬件加速 + WebGPU）；未满足条件时仍显示「点击「新建终端」开始」
...
当 `tabs.length === 0` 时，主内容区由 `EmptyWelcomeView` 接管：

| 条件 | 展示 |
|------|------|
| `advanced.hardwareAcceleration` 与 `advanced.webGpuAcceleration` 均为 `true`，且运行时 WebGPU adapter 可用 | Three.js **WebGPURenderer** 3D NioZy Logo（蓝色「N」+ 圆角终端窗 + 红黄绿按钮），屏幕区循环打字演示命令（含 `niozy --version` 动态输出 Electron / Chromium 版本） |
| 上述任一不满足，或 WebGPU 初始化失败 | 默认文案「点击「新建终端」开始」 |

**交互与视觉**：

- Logo 组整体缩放约 80%，随 idle 轻微浮动
- **鼠标跟随**：指针在欢迎区内移动时 Logo 平滑朝向鼠标；移出区域或窗口失焦后回正
- 底部叠加应用标语（`app.tagline`）与新建终端提示（`app.emptyHint`），`pointer-events-none` 不挡画布交互
- `three` 单独拆包（`renderer-manual-chunks` → `three`），仅在 WebGPU 可用时懒加载

**实现要点**：

- 入口：`src/App.tsx` → `EmptyWelcomeView`
...
`src/components/settings/TerminalSettings.tsx` — 配色、字体、渲染器（DOM/WebGL）、同步渲染、光标、回滚、背景图、右键复制/粘贴与高级右键菜单、**终端闲置动画**。

### 无 Tab 欢迎页与 3D Logo 引擎

- 欢迎视图：`src/components/welcome/EmptyWelcomeView.tsx`、`WelcomeTerminalCanvas.tsx`
- WebGPU 探测：`src/lib/webgpu-capability.ts`
- Three.js 场景：`src/lib/welcome-terminal-engine.ts`（`WelcomeTerminalEngine`；选项 `interactive`、`compact`）
- 依赖：`package.json` → `three`；拆包：`scripts/renderer-manual-chunks.ts`
