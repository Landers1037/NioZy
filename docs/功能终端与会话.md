...
- **无 Tab 欢迎页**（`terminal.welcomePage`）：可开启欢迎页并在 **NioZy 3D** / **像素动画** 间切换；启动无会话恢复时不自动新建终端，关闭全部 Tab 后也会展示；详见 §无 Tab 欢迎页
...
| `animation` | 动画类型：`niozy3d`（3D WebGPU）或 `pixel`（PixiJS 像素动画，默认 `niozy3d`） |
...
| `welcomePage.enabled === true` | `EmptyWelcomeView`，按 `animation` 切换 3D / 像素视图 |
...
| `pixel` | PixiJS v8 **纯代码绘制** 8-bit 像素 mascot（N 标志 + 终端窗 + 发光平台），循环播放分镜：`IDLE_1/2/3` → `HAPPY` → `EXCITED` → `THINKING`（爱心 / 闪光 / 思考气泡） | Pixi 初始化失败 → `EmptyWorkspaceHint` |
...
- 视图：`WelcomePagePixelView` → `WelcomePixelAnimation.tsx`
- 引擎：`src/lib/welcome-pixel-animation-engine.ts`（`WelcomePixelAnimationEngine`；`Graphics` 像素格绘制，无外部 sprite 资源）
- 分层：N 后层竖笔 → 终端窗 → N 前景斜杠（斜杠一段叠在终端左下，保持字母可读）
- 依赖：`package.json` → `pixi.js`；拆包：`scripts/renderer-manual-chunks.ts` → `pixi`
...
- 欢迎视图：`src/components/welcome/EmptyWelcomeView.tsx`（`EmptyWorkspaceHint`、`WelcomePage3DView`、`WelcomePagePixelView`）
- 启动逻辑：`src/App.tsx`（`isWelcomePageEnabled`、跳过默认 `createTerminal()`、`appBootComplete` 后展示）
- 设置类型：`electron/shared/welcome-page-settings.ts`
- **3D**：`WelcomeTerminalCanvas.tsx`、`src/lib/welcome-terminal-engine.ts`；WebGPU 探测：`src/lib/webgpu-capability.ts`
- **像素**：`WelcomePixelAnimation.tsx`、`src/lib/welcome-pixel-animation-engine.ts`
- 依赖与拆包：`three`、`pixi.js` → `scripts/renderer-manual-chunks.ts`
