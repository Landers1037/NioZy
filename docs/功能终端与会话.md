...
- **命令面板**（`Ctrl+Shift+P`）：模糊搜索执行终端 Tab 操作（新建、重命名、关闭、分组、克隆、拆分、导出、截图、切换渲染模式等）；详见 §命令面板
...
| `/all`、`help`、`/help` | 展示全部命令（可用项在前，不可用灰显） |
...
| 切换渲染模式 | DOM ↔ WebGL 热切换 | xterm 引擎（Wterm 下不可用） |

需对话框的命令（重命名、关闭确认、添加到分组、终端截图）会先关闭命令面板再打开对应弹框，与侧栏 Tab 右键菜单共用同一套对话框组件。
...
- 渲染模式切换：`src/lib/terminal-render-actions.ts`（`toggleTerminalRenderMode`）
...
- UI：`src/components/layout/CommandPalette.tsx`（挂载于 `App.tsx`）
- 命令与执行：`src/lib/command-palette-commands.ts`；模糊匹配：`src/lib/command-palette-fuzzy.ts`
- 状态：`src/stores/command-palette-store.ts`；渲染模式切换：`src/lib/terminal-render-actions.ts`
