...
- **命令面板**（`Ctrl+Shift+P`）：模糊搜索执行终端 Tab 操作（新建、重命名、关闭、分组、克隆、拆分、导出、截图、切换渲染模式、切换配色方案等）；详见 §命令面板
...
| `/all`、`help`、`/help` | 展示全部 11 条命令（可用项在前，不可用灰显） |
...
需对话框的命令（重命名、关闭确认、添加到分组、终端截图）会先关闭命令面板再打开对应弹框，与侧栏 Tab 右键菜单共用同一套对话框组件。需子面板的命令（渲染模式、配色方案）保持命令面板打开并切换至二级列表。
...
- 子面板选项与筛选：`src/lib/command-palette-pickers.ts`（渲染模式、配色方案列表）
