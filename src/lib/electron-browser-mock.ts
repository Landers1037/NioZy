...
export function createBrowserDevElectronAPI(): BrowserDevElectronAPI {
  return {
    ...
    p2p: {
      ...
      openConversation: async () => ({ ok: false, error: 'Browser preview', online: false }),
      hideFromSidebar: async () => ({ ok: false, error: 'Browser preview' }),
      removeConversation: async () => ({ ok: false, error: 'Browser preview' }),
      ...
      onConversationHidden: () => () => undefined,
      onMessage: () => () => undefined,
    ...
    workspace: {
      ...
      gitCommit: async () => ({ ok: false as const, error: 'Browser preview' }),
      gitPush: async () => ({ ok: false as const, error: 'Browser preview' }),
      listHistory: async () => [],
      recordHistory: async () => [],
    },
    notes: {
    ...
    muxTerminal: {
      create: async (options) => {
        ...
        return {
          ...
          layoutKind: options.layoutKind ?? '2x2',
        }
      },
      write: () => undefined,
      resize: () => undefined,
      setFocus: () => undefined,
      scroll: () => undefined,
      setResizeMode: async () => false,
      adjustSplit: async () => false,
      closePane: async () => null,
      kill: () => undefined,
      isAlive: async () => true,
      setActiveStreams: () => undefined,
      claimStream: async () => '',
