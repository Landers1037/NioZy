...
import { FolderCode, FolderOpen, X } from 'lucide-react'
import {
  ContextMenu,
  ContextMenuContent,
  ContextMenuItem,
...
interface FilesystemTabItemProps {
  tab: AppTab
  collapsed?: boolean
  iconOnly?: boolean
  isActive: boolean
...
export function FilesystemTabItem({
  tab,
  collapsed = false,
  iconOnly = false,
  isActive,
}: FilesystemTabItemProps) {
  ...
  const row = (
    ...
    >
      <FolderCode className={cn('shrink-0', iconOnly ? 'size-3' : 'size-4')} />
      {!compact && (
        <>
