...
import { PenTool, FolderOpen, X } from 'lucide-react'
import {
  ContextMenu,
  ContextMenuContent,
  ContextMenuItem,
...
import { useDrawingTabClose } from '@/hooks/useDrawingTabClose'

interface ExcalidrawTabItemProps {
  tab: AppTab
  collapsed?: boolean
  iconOnly?: boolean
  isActive: boolean
...
export function ExcalidrawTabItem({
  tab,
  collapsed = false,
  iconOnly = false,
  isActive,
}: ExcalidrawTabItemProps) {
  const { t } = useTranslation()
  const setActiveTab = useAppStore((s) => s.setActiveTab)
  const uiStyle = useUiStyle()
  const { confirmOpen, setConfirmOpen, requestClose, confirmClose } = useDrawingTabClose(
    tab.id,
    'excalidraw',
  )
  ...
  const row = (
    ...
    >
      <PenTool className={cn('shrink-0', iconOnly ? 'size-3' : 'size-4')} />
      {!compact && (
        <>
