...
import { useMarkdownTabClose } from '@/hooks/useMarkdownTabClose'

interface MarkdownTabItemProps {
  tab: AppTab
  collapsed?: boolean
  iconOnly?: boolean
  isActive: boolean
...
export function MarkdownTabItem({
  tab,
  collapsed = false,
  iconOnly = false,
  isActive,
}: MarkdownTabItemProps) {
  const { t } = useTranslation()
  const setActiveTab = useAppStore((s) => s.setActiveTab)
  const uiStyle = useUiStyle()
  const { confirmOpen, setConfirmOpen, requestClose, confirmClose } = useMarkdownTabClose(tab.id)
  ...
  const row = (
    ...
    >
      <FileText className={cn('shrink-0', iconOnly ? 'size-3' : 'size-4')} />
      {!compact && (
        <>
